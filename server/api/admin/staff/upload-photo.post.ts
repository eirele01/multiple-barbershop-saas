/**
 * POST /api/admin/staff/upload-photo
 *
 * Upload a staff member's profile photo to Supabase Storage.
 * Storage path: shop-assets/[shopId]/staff/[filename]
 *
 * Validates:
...
export default defineEventHandler(async (event) => {
  ...
  for (const part of formData) {
    if (part.name === 'file' && part.filename) {
      ...
      if (part.data.length > MAX_FILE_SIZE) {
        throw createError({
          statusCode: 413,
          statusMessage: 'File too large. Maximum size is 5MB for staff photos.',
        })
      }
  ...
  // Upload to Supabase Storage in shop-assets bucket
  const filePath = `shop-assets/${userProfile.shop_id}/staff/${fileName}`
  ...
  if (uploadError) {
    throw createError({ statusCode: 500, statusMessage: 'Failed to upload photo: ' + uploadError.message })
  }
